...
export default function BuildingCard({ building, shown, active, onSelect }: BuildingCardProps) {
  ...
  const [isFav, setIsFav] = useState<boolean>(FavoritesService.isFavoriteBuilding(building.id));
  ...
  const toggleFavorite = (e: React.MouseEvent) => {
    e.stopPropagation();
    FavoritesService.toggleBuilding(building.id);
    setIsFav(FavoritesService.isFavoriteBuilding(building.id));
  };
  return (
    <>
      <Card className={`building-card ${shown ? "show" : ""} ${isActive ? "active" : ""}`}
        onClick={() => {
          ...
          // На мобильном: сворачиваем оверлей таб-контента, чтобы показать карту
          if (window.innerWidth <= 768) {
            try { document.body.classList.add('mobile-overlay-hidden'); } catch {}
          }
        }}>
        <img
          ...
          <div>
            <span className="building-card-text-head">
              ...
              <span className="row gap8">
                <button className="bookmark-btn" onClick={toggleFavorite} title={isFav ? "Убрать из закладок" : "В закладки"}>
                  <FontAwesomeIcon icon={faBookmark} style={{ color: isFav ? 'var(--kutg-color)' : 'rgba(0,0,0,0.2)' }} />
                </button>
                <img src={"/assets/type" + building.type + ".png"}></img>
              </span>
            </span>
